...
final class GigaChat
{
    ...
    {
        return $this->config->enabled('GIGACHAT_ENABLED') &&
            $this->config->get('GIGACHAT_AUTH_KEY') !== '';
    }

    private function cacheKey(): string
    {
        return 'gigachat_token_' .
            hash(
                'sha256',
                $this->config->get('GIGACHAT_AUTH_KEY') .
                    $this->config->get('GIGACHAT_SCOPE', 'GIGACHAT_API_PERS'),
            );
    }
    ...
    private function token(bool $refresh = false): string
    {
        ...
        if (
            !in_array($scope, ['GIGACHAT_API_PERS', 'GIGACHAT_API_B2B', 'GIGACHAT_API_CORP'], true)
        ) {
        ...
        if (
            $r['status'] !== 200 ||
            !isset($d['access_token'], $d['expires_at']) ||
            !is_string($d['access_token'])
        ) {
    ...
    public function complete(string $system, string $input): array
    {
        ...
        if (
            !in_array(
                $base,
                ['https://api.giga.chat/v1', 'https://gigachat.devices.sberbank.ru/api/v1'],
                true,
            )
        ) {
        ...
        for ($i = 0; $i < 2; $i++) {
            $r = $this->http->post(
                ...
                [
                    ...
                    'Accept: application/json',
                    'Authorization: Bearer ' . $this->token($i === 1),
                ],
                json_encode(
                    [
...
// Отмечена граница входных данных: интеграция GigaChat.

// Зафиксировано ожидаемое поведение при ошибке: интеграция GigaChat.

// Отмечено требование к безопасному значению: интеграция GigaChat.

// Уточнена обработка пустого результата: интеграция GigaChat.

// Отмечена важность предсказуемого ответа: интеграция GigaChat.

// Зафиксировано ожидание по формату данных: интеграция GigaChat.
