...
final class Database
{
    ...
    public function initialize(): void
    {
        ...
        $this->transaction(function (): void {
            ...
            foreach (['users' => ['timezone_confirmed', 'telegram_blocked', 'ai_consent'], 'tasks' => ['revision']] as $table => $columns) {
                $existing = array_column($this->run('PRAGMA table_info(' . $table . ')')->fetchAll(), 'name');
                foreach ($columns as $column) {
                    if (!in_array($column, $existing, true)) {
                        $this->pdo->exec("ALTER TABLE $table ADD COLUMN $column INTEGER NOT NULL DEFAULT 0");
                    }
                }
            }
            $this->pdo->exec("CREATE TABLE IF NOT EXISTS sessions (user_id INTEGER PRIMARY KEY REFERENCES users(telegram_id), data TEXT NOT NULL, updated_at INTEGER NOT NULL);
    ...
    {
        $this->run('INSERT OR IGNORE INTO updates(id,payload,received_at) VALUES(?,?,?)', [$update['update_id'], json_encode($update, JSON_UNESCAPED_UNICODE | JSON_THROW_ON_ERROR), time()]);
    }
    public function state(int $id): array
    {
        $r = $this->one('SELECT * FROM sessions WHERE user_id=?', [$id]);
        return $r && (int)$r['updated_at'] > time() - 86400 ? json_decode($r['data'], true, 64, JSON_THROW_ON_ERROR) : [];
    }
    public function saveState(int $id, array $s): void
    {
        $this->run('INSERT INTO sessions(user_id,data,updated_at) VALUES(?,?,?) ON CONFLICT(user_id) DO UPDATE SET data=excluded.data,updated_at=excluded.updated_at', [$id, json_encode($s, JSON_UNESCAPED_UNICODE | JSON_THROW_ON_ERROR), time()]);
    }
    ...
    {
        $this->run('INSERT OR IGNORE INTO outbox(method,payload,chat_id,task_id,revision,reminder_hours) VALUES(?,?,?,?,?,?)', [$method, json_encode($payload, JSON_UNESCAPED_UNICODE | JSON_THROW_ON_ERROR), $payload['chat_id'] ?? null, $reminder['id'] ?? null, $reminder['revision'] ?? null, $reminder['hours'] ?? null]);
    }
    public function tasks(int $id, bool $active = true): array
    {
        return $this->run('SELECT * FROM tasks WHERE user_id=?' . ($active ? " AND status='active'" : '') . ' ORDER BY deadline,id', [$id])->fetchAll();
    }
    public function task(int $user, int $id): ?array
    {
        return $this->one("SELECT * FROM tasks WHERE user_id=? AND id=? AND status='active'", [$user, $id]);
    }
    public function create(int $user, array $draft, \DateTimeImmutable $now, bool $demo = false): void
    {
        Dates::validate($draft, $now);
        $this->run('INSERT INTO tasks(user_id,title,subject,deadline,estimated_minutes,importance,created_at,is_demo) VALUES(?,?,?,?,?,?,?,?)', [$user, $draft['title'], $draft['subject'] ?? null, $draft['deadline'], $draft['estimated_minutes'], $draft['importance'], Dates::utc($now), (int)$demo]);
    }
}
