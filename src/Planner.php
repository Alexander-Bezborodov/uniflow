...
final class Planner
{
    public static function priority(array $t, \DateTimeImmutable $now): int
    {
        ...
        return min(4, $base + ((int)$t['importance'] === 3 || (int)$t['estimated_minutes'] >= 180 ? 1 : 0));
    }
    ...
    public static function plan(array $tasks, \DateTimeImmutable $now): array
    {
        usort($tasks, function ($a, $b) use ($now): int {
            return [ -self::priority($a, $now),strtotime($a['deadline']),-(int)$a['importance'],-(int)$a['estimated_minutes'],(int)$a['id']] <=> [-self::priority($b, $now),strtotime($b['deadline']),-(int)$b['importance'],-(int)$b['estimated_minutes'],(int)$b['id']];
        });
        foreach ($tasks as &$t) {
    ...
    {
        $priority = ['','🟢 Низкий','🟡 Средний','🟠 Высокий','🔴 Очень высокий','🔴 Просрочено'];
        return '<b>' . Emojis::escape($t['title']) . "</b>\nПредмет: " . Emojis::escape($t['subject'] ?? 'не указан') . "\n📅 " . (isset($t['deadline']) ? Dates::label($t['deadline'], $now) : 'Уточни дедлайн') . "\n⏱ " . ($t['estimated_minutes'] ?? '?') . " мин · Важность: " . ($t['importance'] ?? 2) . '/3' . (isset($t['id']) ? "\n" . $priority[self::priority($t, $now)] : '');
    }
    ...
    public const REASONS = ['deadline' => 'Срок близко. Начни с этой задачи.','prepare' => 'Начни заранее, чтобы распределить нагрузку.','importance' => 'У задачи высокая важность.','volume' => 'Задача объёмная. Лучше начать заранее.'];
}
